...
import TwoImg from './email.png';
import OneImg from './github.png';
import ThreeImg from './github2.jpg';


<Callout variant="warning">
This project was implemented as an educational DevOps exercise using publicly available data from a job portal that had no scraping prohibitions in its robots.txt file or Terms of Service at the time of development. This implementation shouldn't be interpreted as endorsement of web scraping - conduct your own legal assessment for similar projects.
</Callout>
...
I wanted a system that could automatically look for job listings from a website (in this case an IT job portal in Croatia).
Parse and filter these listings for new job opportunities and then send me an email notification if there were any new jobs that hadn't been seen before.
...
## Step 1: Finding Job Listings with Node.js
...
const URL = 'https://website.hr/pretraga?category=it-poslovi&province=zagreb&activated_from=all&min_hour_rate=';
...
<Image src={TwoImg} alt="Company Network Layout" />
...
The GitHub Actions workflow file (find-jobs.yml) looks like this:
```bash title="find-jobs.yml"
    ...
    steps:
      ...
      - name: Commit and push updated jobs file
        run: |
          ...
          git push https://x-access-token:${{ secrets.GH_TOKEN }}@github.com/{user}/studentjobs.git
        env:
...
<Image src={OneImg} alt="GHActions" />